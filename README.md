...
- **PrivateLink / VPC Endpoints**: CloudPilot backend can be deployed inside an AWS VPC with VPC Endpoints (AWS PrivateLink), allowing API calls to never traverse the public internet. Upon connecting your AWS credentials, the application will present an automated prompt asking if you'd like the agent to automatically configure the required VPC endpoints and security groups in your account. The agent will gracefully report any missing IAM permissions needed to complete the setup and allow you to easily re-submit your credentials.
